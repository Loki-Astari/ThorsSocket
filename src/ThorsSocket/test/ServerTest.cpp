...
TEST(ServerTest, SecureServerCreate)
{
    ...
    Server          server{SServerInfo{port, ctx, std::move(certificate)}, Blocking::Yes, [](){return false;}, [](){return false;}};
}
...
TEST(ServerTest, SecureserverAcceptConnection)
{
    ...
    Socket  socket({"127.0.0.1", port, ctxClient, std::move(certificateClient)});
    ASSERT_NE(0, socket.socketId(Mode::Read));
